...
TABLE_MODEL_MAP: dict[str, tuple[type, set[str]]] = {
    "ContractingParty": (ContractingPartyWrite, {"id"}),
    "ContractFolderStatus": (ContractFolderStatusWrite, {"id", "created_at"}),
    "ProcurementProjectLot": (ProcurementProjectLotWrite, {"id"}),
    "CpvClassification": (CpvClassificationWrite, {"id"}),
    "RealizedLocation": (RealizedLocationWrite, {"id"}),
    "TenderResult": (TenderResultWrite, {"id"}),
    "WinningParty": (WinningPartyWrite, {"id"}),
    "Contract": (ContractWrite, {"id"}),
    "AwardingCriteria": (AwardingCriteriaWrite, {"id"}),
    "FinancialGuarantee": (FinancialGuaranteeWrite, {"id"}),
    "QualificationRequirement": (QualificationRequirementWrite, {"id"}),
    "BusinessClassification": (BusinessClassificationWrite, {"id"}),
    "ExecutionCondition": (ExecutionConditionWrite, {"id"}),
    "ValidNoticeInfo": (ValidNoticeInfoWrite, {"id"}),
    "PublicationStatus": (PublicationStatusWrite, {"id"}),
    "DocumentReference": (DocumentReferenceWrite, {"id"}),
    "ContractModification": (ContractModificationWrite, {"id"}),
    "StatusChange": (StatusChangeWrite, {"id", "recorded_at"}),
    "EtlSyncState": (EtlSyncStateWrite, {"id"}),
    "EtlFailedEntries": (EtlFailedEntryWrite, {"id"}),
}
