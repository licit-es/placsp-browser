...
async def pool():
    ...
    yield p
    await p.execute('TRUNCATE "EtlFailedEntries" CASCADE')
    await p.close()
...
class TestRecordFailure:
    ...
    async def test_inserts_new_failure(
        ...
        row = await pool.fetchrow(
            'SELECT * FROM "EtlFailedEntries" WHERE entry_id = $1',
            "test://entry1",
        )
    ...
    async def test_updates_last_failed_at(
        ...
        row1 = await pool.fetchrow(
            'SELECT last_failed_at FROM "EtlFailedEntries" WHERE entry_id = $1',
            "test://entry1",
        )
        await repo.record_failure(
            "outsiders",
...
class TestMarkResolved:
    ...
    async def test_sets_resolved_at(self, repo: PgFailedEntryRepository, pool) -> None:
        ...
        row = await pool.fetchrow(
            'SELECT resolved_at FROM "EtlFailedEntries" WHERE entry_id = $1',
            "test://entry1",
        )
    ...
    async def test_resolved_allows_new_failure(
        ...
        count = await pool.fetchval(
            'SELECT count(*) FROM "EtlFailedEntries" WHERE entry_id = $1',
            "test://entry1",
        )
        assert count == 2
