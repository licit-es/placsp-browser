...
async def pool():
    ...
    yield p
    await p.execute('TRUNCATE "ContractFolderStatus", "ContractingParty" CASCADE')
    await p.close()
...
class TestContractingParty:
    ...
    async def test_party_created(self, repo: PgEntryRepository, pool) -> None:
        ...
        await repo.process_entry(entry)
        row = await pool.fetchrow(
            'SELECT name, dir3 FROM "ContractingParty" WHERE dir3 = $1',
    ...
    async def test_dir3_identity_resolution(
        ...
        count = await pool.fetchval(
            'SELECT count(*) FROM "ContractingParty" WHERE dir3 = $1',
            "EA0003089",
        )
        assert count == 1
    ...
    async def test_platform_id_identity_resolution(
        ...
        count = await pool.fetchval(
            'SELECT count(*) FROM "ContractingParty" WHERE platform_id = $1',
            "PLAT001",
        )
        assert count == 1
    ...
    async def test_platform_id_identity_resolution_different_dir3(
        ...
        assert count == 1
        row = await pool.fetchrow(
            'SELECT dir3, name FROM "ContractingParty" WHERE platform_id = $1',
            "PLAT002",
        )
    ...
    async def test_name_fallback_identity(self, repo: PgEntryRepository, pool) -> None:
        ...
        count = await pool.fetchval(
            'SELECT count(*) FROM "ContractingParty"'
            " WHERE name = $1"
            " AND dir3 IS NULL"
    ...
    async def test_stale_entry_does_not_touch_party(
        ...
        row = await pool.fetchrow(
            'SELECT name FROM "ContractingParty" WHERE dir3 = $1',
            "EA0003089",
        )
    ...
    async def test_party_merge_on_conflict(self, repo: PgEntryRepository, pool) -> None:
        ...
        pre_count = await pool.fetchval('SELECT count(*) FROM "ContractingParty"')
        assert pre_count == 2
        ...
        post_count = await pool.fetchval('SELECT count(*) FROM "ContractingParty"')
        assert post_count == 1

        merged = await pool.fetchrow(
            'SELECT name, dir3, platform_id FROM "ContractingParty"'
        )
    ...
    async def test_entry_persisted_when_party_fails(
        ...
        assert result.status == "ok"
        row = await pool.fetchrow(
            'SELECT contracting_party_id FROM "ContractFolderStatus"'
            " WHERE entry_id = $1",
            "test://fail1",
        )
        assert row is not None
        ...
        sc = await pool.fetchval(
            'SELECT count(*) FROM "StatusChange" sc'
            ' JOIN "ContractFolderStatus" cfs ON sc.contract_folder_status_id = cfs.id'
...
class TestChildReplacement:
    ...
    async def test_lots_replaced_on_reupsert(
        ...
        count = await pool.fetchval('SELECT count(*) FROM "ProcurementProjectLot"')
        assert count == 1
    ...
    async def test_results_and_parties_replaced(
        ...
        count = await pool.fetchval('SELECT count(*) FROM "TenderResult"')
        assert count == 0
        wp_count = await pool.fetchval('SELECT count(*) FROM "WinningParty"')
        assert wp_count == 0
...
class TestStatusChange:
    ...
    async def test_status_logged(self, repo: PgEntryRepository, pool) -> None:
        ...
        count = await pool.fetchval('SELECT count(*) FROM "StatusChange"')
        assert count == 1
...
class TestFullParsedEntry:
    ...
    async def test_full_entry_roundtrip(self, repo: PgEntryRepository, pool) -> None:
        ...
        lot_count = await pool.fetchval('SELECT count(*) FROM "ProcurementProjectLot"')
        assert lot_count == 1

        tr_count = await pool.fetchval('SELECT count(*) FROM "TenderResult"')
        assert tr_count == 1

        tr = await pool.fetchrow('SELECT lot_id FROM "TenderResult"')
        assert tr["lot_id"] is not None

        wp_count = await pool.fetchval('SELECT count(*) FROM "WinningParty"')
        assert wp_count == 1

        doc_count = await pool.fetchval('SELECT count(*) FROM "DocumentReference"')
        assert doc_count == 1

        cpv_count = await pool.fetchval('SELECT count(*) FROM "CpvClassification"')
        assert cpv_count == 2
